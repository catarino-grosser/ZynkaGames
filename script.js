...
async function carregarAventurasDoCatalogo() {
  ...
  const modulos = await Promise.all(
    aventuraArquivos.map(arquivo => import(arquivo).then(modulo => ({ arquivo, modulo })))
  );

  for (const { arquivo, modulo } of modulos) {
...
function normalizeSave(saved) {
  ...
  saved.solvedPuzzles = saved.solvedPuzzles || [];
  return saved;
}
...
$('startAdventureBtn').onclick = () => {
  ...
  state.game = {
    ...
    openSideQuests: [], completedSideQuests: [], discoveredTreasures: [], visitedRandomEvents: [], solvedPuzzles: []
  };
...
function renderGame() {
  ...
  if (chapter.puzzle && !state.game.solvedPuzzles.includes(state.game.chapterId)) renderPuzzle(chapter);
...
function renderPuzzle(chapter) {
  ...
  b.onclick = () => {
    ...
    if (ok) {
      ...
      state.game.solvedPuzzles.push(state.game.chapterId);
...
function renderBattle(chapter) {
  ...
  addBattleButton('🛡 Defender', '', () => enemyTurn(enemy, 'Você se defendeu.', 3));
  renderConsumableButtons();
}

function renderConsumableButtons() {
  const adv = getAdventure();
  const h = state.game.hero;
  h.inventory.forEach(invItem => {
    const def = adv.items?.[invItem.id];
    if (def?.heal && invItem.qty > 0) {
      addBattleButton(`🧪 ${def.name} (+${def.heal})`, 'reward', () => useConsumable(invItem.id));
    }
  });
}
...
function enemyTurn(enemy, prefix = '', tempDef = 0) {
  const h = state.game.hero;
  const damage = Math.max(1, enemy.atk - (h.def + tempDef));
  h.hp -= damage;
